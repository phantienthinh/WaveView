...
class WaveView @JvmOverloads constructor(
    ...
    companion object {
        private const val BORDER_ROW = 5 //dp
        private const val midPoint = 18 //dp
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.drawTopRoundRect(
            rowRect1,
            convertDp2Px(BORDER_ROW, context), rowPain

        )
        canvas.drawTopRoundRect(
            rowRect2,
            convertDp2Px(BORDER_ROW, context)
            , rowPain
        )
        canvas.drawTopRoundRect(
            rowRect3,
    ...
    private fun initRectRow() {
        rowRect1 = RectF(
            spaceDraw - convertDp2Px(midPoint, context),
            valueRow1,
            spaceDraw + convertDp2Px(midPoint, context), mHeightView.toFloat()
        )
        rowRect2 = RectF(
            3 * spaceDraw - convertDp2Px(midPoint, context),
            valueRow2,
            3 * spaceDraw + convertDp2Px(midPoint, context), mHeightView.toFloat()
        )
        rowRect3 = RectF(
            5 * spaceDraw - convertDp2Px(midPoint, context),
            valueRow3,
            5 * spaceDraw + convertDp2Px(midPoint, context), mHeightView.toFloat()
        )
    }
    ...
    fun createAnim1(startValue: Float, endValue: Float) {
        animation1.cancel()
    ...
    fun createAnim2(startValue: Float, endValue: Float) {
        animation2.cancel()
    ...
    fun createAnim3(startValue: Float, endValue: Float) {
        animation3.cancel()
